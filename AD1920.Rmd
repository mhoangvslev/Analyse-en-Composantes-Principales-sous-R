...
plotACP <- function(resultACP, attribut = c("var", "ind"), prp = c("qlt", "ctr", "coords"), x, y, clr, titre, viridis_palette = "D"){
  ...
  dfclr <- if(length(clr) == 1) dfclr <- as.data.frame(resultACP[[attribut]][[prp]])[[clr]] else clr;
  dfclrName <- if(length(clr) == 1) clr else "Groupes";

  plt <- ggplot(df, mapping = aes(x = df[[x]], y = df[[y]], color = dfclr)); 
  plt <- plt + geom_point() 
  
  if(length(clr) == 1) plt <- plt + scale_color_viridis(option = viridis_palette);
...
plotACP(testACP, attribut = "ind", "ind", x = "Comp. 1", y = "Comp. 2", prp = "coords", clr = indCorClrs, titre = "Position des individus")
...
plotACP(testACP, attribut = "ind", x = "Comp. 1", y = "Comp. 2", prp = "ctr", clr = "Comp. 1", titre = "Contribution des variables sur Axe 1", viridis_palette = "plasma");

plotACP(testACP, attribut = "ind", x = "Comp. 1", y = "Comp. 2", prp = "ctr", clr = "Comp. 2", titre = "Contribution des variables sur Axe 2", viridis_palette = "plasma");

...
```{r}
testACP <- ACP(t(genes), standardisation = FALSE, nbComps = 3);
```
...
plot(testACP$var$coords, 
     main = "Corrélation entre les composantes principales et les variables",
     col = varClrs)

library(leaflet)
varQltClrs <- sapply(testACP$var$qlt, colorNumeric(palette = "magma", domain = testACP$var$qlt));
plot(testACP$var$coords, 
     main = "Contribution relative des variables sur les axes",
     col = varQltClrs)

varCtrClrs <- sapply(testACP$var$ctr, colorNumeric(palette = "magma", domain = testACP$var$ctr));
plot(testACP$var$coords, 
     main = "Contribution absolue des variables sur les axes",
     col = varCtrClrs)
```
...
indCorClrs <- sapply(1:nrow(testACP$ind$coords), function(id){
  ifelse(id <= 20, rgb(0,1,0,1), rgb(1,0,0,1))
})
...
library(leaflet)
indQltClrs <- sapply(testACP$ind$qlt, colorNumeric(palette = "magma", domain = testACP$ind$qlt));
plot(testACP$ind$coords, 
     main = "Contribution relative des individus sur les axes",
     col = indQltClrs)

indCtrClrs <- sapply(testACP$ind$ctr, colorNumeric(palette = "viridis", domain = testACP$ind$ctr));
plot(testACP$ind$coords, 
     main = "Contribution absolue des individus sur les axes",
     col = indCtrClrs)
```
